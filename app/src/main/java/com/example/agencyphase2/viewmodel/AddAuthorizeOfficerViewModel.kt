package com.example.agencyphase2.viewmodel

import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.agencyphase2.model.pojo.add_authorize_officer.AddAuthorizeOfficerResponse
import com.example.agencyphase2.model.repository.AddAuthorizeOfficerRepository
import com.example.agencyphase2.model.repository.Outcome
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.flow.catch
import kotlinx.coroutines.flow.onStart
import kotlinx.coroutines.launch
import javax.inject.Inject

@HiltViewModel
class AddAuthorizeOfficerViewModel @Inject constructor(private val repository: AddAuthorizeOfficerRepository) : ViewModel() {
    private var _response = MutableLiveData<Outcome<AddAuthorizeOfficerResponse?>?>()
    val response: LiveData<Outcome<AddAuthorizeOfficerResponse?>?> = _response

    fun addAuthorizeInfo(
        fullname: String,
        email: String,
        phone: String,
        role : String,
        token: String
    ) = viewModelScope.launch {
        repository.addAuthorizeOfficer(
            fullname, email, phone, role, token
        ).onStart {
            _response.value = Outcome.loading(true)
        }.catch {
            _response.value = Outcome.Failure(it)
        }.collect {
            _response.value = Outcome.success(it)
        }
    }

    fun navigationComplete(){
        _response.value = null
    }
}